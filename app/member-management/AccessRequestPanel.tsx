  ...
  return (
    <div className="rounded-xl border border-border p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="font-medium text-foreground">{request.name || request.email}</p>
      ...
      {request.message && (
        <p className="mt-2 rounded-lg bg-accent px-3 py-2 text-sm text-muted-foreground">
          "{request.message}"
        </p>
      )}
      ...
      {request.status === "pending" ? (
        <>
          <p className="mt-3 mb-1 text-xs font-medium text-muted-foreground">요청한 게시판 (승인 전 조정 가능)</p>
...
export default function AccessRequestPanel() {
  ...
  return (
    <div className="mb-8">
      <h2 className="mb-3 text-xl font-semibold text-foreground">게시판 이용 신청 관리</h2>
